...
- (void)setupDisclosureButton {
    ...
    UIImageView *imag = [[UIImageView alloc]initWithFrame:CGRectMake(120, +26, 35, 35)];
    imag.image=[UIImage imageNamed:@"right_arrow12"];
    ...
    _disclosureButton.frame = CGRectMake(-3, -8,self.titleLabel.frame.size.width, self.frame.size.height);
    
    [_disclosureButton addTarget:self action:@selector(didTapDisclosureButton) forControlEvents:UIControlEventTouchDown];
    [_disclosureButton addSubview:imag];
...
- (void)setDetailGroupAlpha:(CGFloat)alpha {
    // self.disclosureButton.frame = CGRectMake(10, 0, 40, 10)
